...
ModuleStack::ModuleStack(unsigned int maxSize) : Count(0), Modules( new HMODULE[maxSize] ) {
	DWORD MachineType;
	CONTEXT Context;
  ...
  HMODULE module = (HMODULE)-1;
	...
	while ( Count < 256 && StackWalk64(MachineType,GetCurrentProcess(),GetCurrentThread(),&StackFrame,MachineType == IMAGE_FILE_MACHINE_I386 ? NULL: &Context, NULL,SymFunctionTableAccess64, SymGetModuleBase64, NULL ) && StackFrame.AddrPC.Offset != 0 ) {
		...
		if( module != lastModule && module != Kernel32Module && module != NTDLLModule ) {
			lastModule = Modules[Count++] = module;
		}
	}
